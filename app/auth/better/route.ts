...
const normalizeNextPath = (value?: string | null) => {
  ...
  try {
    ...
    console.warn('Invalid next path provided for Better Auth:', value, error);
    return undefined;
  }
};
...
const getRedirectTarget = (
  ...
  if (options?.next) {
    target.searchParams.set('next', options.next);
  }
  ...
  switch (intent) {
    case 'reset': {
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(normalizeEmail(email), {
        redirectTo: getRedirectTarget(env, redirectTo, request, { next: nextPath }),
      });
    ...
    case 'signup': {
      ...
      const emailRedirectTo = getRedirectTarget(env, redirectTo, request, { next: nextPath });
