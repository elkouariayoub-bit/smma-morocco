...
export function SignIn({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.replace(result.redirect ?? postAuthRedirect ?? '/dashboard');
      router.refresh();
    } catch (error) {
  ...
  const handleOAuth = useCallback(async () => {
    ...
    [oauthRedirect, postAuthRedirect]
  );
...
const SUPABASE_CONFIG_MESSAGE =
  'Supabase credentials are not configured. Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to enable account features.';
...
export function UserButton({ onSignOut, hasCodeSession = false }: UserButtonProps) {
  ...
  useEffect(() => {
    if (!supabase) {
      const email = hasCodeSession ? 'access@smma-morocco.com' : null;
      setUserEmail(email);
